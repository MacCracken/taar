...
- **x86_64**: smoke → `taar smoke ok`; `cyrius tests tests/` → 73 passed,
  0 failed; `resolve-smoke` resolved a live domain over UDP-53.
- **aarch64** (`qemu-aarch64`): `cyrius tests --aarch64` → 73 passed, 0 failed;
  smoke ok; `resolve-smoke` resolved a live domain. The same check against
...
- **AGNOS `taar_tcp_recv` conflates timeout with EOF.** A deadline expiry
  ...
  transport — schedule it with a whirl cut, not a taar patch.
...
- No CI job builds `--aarch64`, so the sendto defect could ship. Consider
  adding an aarch64 leg (qemu-user is enough — it caught this one).
